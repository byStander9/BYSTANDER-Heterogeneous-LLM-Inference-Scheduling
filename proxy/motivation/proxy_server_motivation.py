...
class ProxyServer:
    ...
    def __init__(self, backend_servers: List[Dict], metrics_csv_path: str = None):
        ...
        if self.routing_algorithm == "fisher_jenks_sqf":
            ...
            self.fj_stats = {
                ...
                'total_candidates_selected': 0,  # 후보 그룹에 포함된 총 횟수
                'fj_fallback_used': 0,  # 기본 threshold 사용 횟수 (샘플 부족)
            }
    ...
    def _calculate_weights_from_rr(self, qps, dataset=None) -> list:
        ...
        base_dir = Path(__file__).parent.parent / f"regressor_results_{num_gpus}gpu"
        if dataset:
    ...
    def emergency_save(self):
        ...
        # 디렉토리 구성
        num_gpus = len(self.backend_servers)
        base_results_dir = Path(__file__).parent.parent / f"regressor_results_{num_gpus}gpu"
    ...
    async def _update_slm_mode(self):
        ...
        if not self.slm_active and avg_e2e > self.slm_activation_threshold:
            ...
            # Fisher-Jenks: 모드 전환 시 diff 버퍼 리셋
    ...
    async def _select_server_fisher_jenks(self, request_data: Optional[Dict] = None) -> tuple:
        ...
        try:
            ...
            # Step 4: Fisher-Jenks split point 갱신
            ...
            # Step 5: 후보 GPU 종류 그룹 선정
            # 1등 GPU 종류는 항상 후보에 포함
            ...
            logger.info(f"[FJ-SQF][{self.request_count}] "
                       ...
                       f"split_point={split_point:.4f}s | "
...
async def set_algorithm(request: Request):
    ...
    try:
        ...
        if new_algorithm == "fisher_jenks_sqf":
            ...
            logger.info(f"[FJ] Fisher-Jenks SQF initialized: window={proxy.fj_window_size}, "
                       f"min_samples={proxy.fj_min_samples}, default_threshold={proxy.fj_default_threshold}")
...
async def get_stats():
    ...
    if proxy.routing_algorithm == "weighted_round_robin":
        ...
        stats["fj_split_history"] = proxy.fj_split_history[-10:]  # 최근 10개
...
async def finalize_experiment(request: Request):
    ...
    try:
        ...
        if algorithm_name == "slm_adaptive":
            ...
            # Fisher-Jenks SQF의 경우 설정값을 파일명에 추가
            act_threshold = getattr(proxy, 'slm_activation_threshold', 15.0)
            deact_threshold = getattr(proxy, 'slm_deactivation_threshold', 10.0)
            ...
            logger.info(f"[Finalize] Fisher-Jenks suffix: {threshold_suffix}")
        ...
        # 결과 저장 디렉토리 설정: regressor_results_{num_gpus}gpu/used_{dataset}/QPS{qps}_{알고리즘명}/
